...
do{
  ...
  output.createOrReplaceTempView("temp_data")
    ...
    if(a == 1){
      ...
      val country_max = hiveCtx.sql("SELECT count(country) as totalsighting, country from ufo_data_part group by country order by totalsighting desc limit 1;")
      country_max.show()

    }
     if(a == 2){
       
     val max_encounter = hiveCtx.sql("Select MAX(duration_seconds) as encountertime, state from ufo_data_part WHERE country = 'us' group by state order by  encountertime DESC limit 5;")
      max_encounter.show()
     ...
     if(a == 3){
      ...
      val ufo_shape = hiveCtx.sql("SELECT Count(shape) as totalshape ,shape  FROM ufo_data_part group by shape order by totalshape desc limit 5;")
      ufo_shape.show();
    ...
    if (a ==4){
     val city_shape = hiveCtx.sql("SELECT AVG(duration_seconds) FROM ufo_data_part WHERE state = 'nj';")
    city_shape.show()
    //WHAT IS THE AVERAGE TIME SOMEONE HAS SEEN A UFO IN NEW JERSEY
    }
    
  if (a==5){
    val total_time = hiveCtx.sql("SELECT Min(shape) as possibleshape, shape from ufo_data_part group by shape order by possibleshape ASC limit 2; ")
    total_time.show();
  ...
  if(a==6){
    val least_likely = hiveCtx.sql("SELECT count(state) as leastlikely,state from ufo_data_part where country = 'us'  group by state order by leastlikely ASC limit 5; ")
    least_likely.show()
